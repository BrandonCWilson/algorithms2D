...
{
    if (!space)return;	
	if (space->spaceHash != NULL)
		spacehash_delete(space->spaceHash);
...
Space *gf2d_space_new()
{
	...
	space->spaceHash = spacehash_new(1024);
    return space;
}
...
//    gf2d_body_adjust_collision_bounds_velocity(body,space->slop,space->bounds,&velocity);
    ...
    for (attempts = 0;attempts < space->precision;attempts++)
    {
		...
		// consolidate the lists from our hashlist
		// might want to move this code to another area later, not entirely sure when it's appropriate to generate the list
		//slog("Consolidating the lists!");
		collisionList = gf2d_list_new();
		if (!collisionList)
		{
			slog("Unable to allocate space for the collisionList");
			return;
		}
		if (body->hashList == NULL)
		{
			//slog("This body doesn't have a hash list! Cannot properly check it for collisions....");
		}
		else
		{
			for (i = 0; i < 9; i++)
			{
				...
				// add the entire list of bodies from the space->spaceHash list, to this list
				if (body->hashList[i] == -1)
				{
					//slog("Skipping a hash because it would be negative");
					continue;
				}
				
				for (j = 0; j < space->spaceHash->entries[body->hashList[i]]->size; j++)
				{
					if (space->spaceHash->entries[body->hashList[i]]->elements[j].data == NULL)
					{
						continue;
					}
					if (body->hashList[i] >= space->spaceHash->maxentries)
					{
						slog("This is probably the issue?");
					}
					collisionList = gf2d_list_append(collisionList, space->spaceHash->entries[body->hashList[i]]->elements[j].data);
					if (!collisionList)
					{
						slog("Got back a null collisionlist");
					}
				}
			}
			free(body->hashList);
		...
		//slog("Lists have been consolidated!");
// based on "body" generate a list of bodies of potential conflict based on your algorith of choice
        for (i = 0; i < bodies; i++)
        {
            other = (Body*)gf2d_list_get_nth(collisionList,i); // use your list instead of the space body list here
		...
		gf2d_list_delete(collisionList);
...
void gf2d_space_update(Space *space)
{
    ...
    for (s = 0; s < 1; s += space->timeStep)
    {
		if (space->spaceHash != NULL)
			spacehash_delete(space->spaceHash);
		space->spaceHash = NULL;
		space->spaceHash = spacehash_new((int)((space->bounds.h / 20)*(space->bounds.w / 20)));
		if (space->spaceHash == NULL)
		{
			slog("spacehash is null. uh oh");
		}
		for (i = 0; i < bodies; i++)
		{
			body = (Body*)gf2d_list_get_nth(space->bodyList, i);
			...
			if (body->hashList != NULL)
				free(body->hashList);
			body->hashList = spacehash_insert(body->position, body, space->spaceHash, space->bounds.w);
			if (body->hashList == NULL)
			{
				slog("We didn't get a hashlist back!");
			}
		}
